...
export function RecruitGuard({ requiredRole, children }: RecruitGuardProps) {
  ...
  useEffect(() => {
    ...
    // Signed in but profile fetch failed (backend down / CORS / 500).
    // DO NOT redirect to login — that creates an infinite loop because
    // the Clerk <SignIn> component auto-redirects authenticated users
    // back to the dashboard, cycling forever.
    if (!recruitProfile) return;

    // Wrong role — redirect to the right home page.
  ...
  // Authenticated but backend unreachable — show retry screen instead of looping.
  if (!recruitProfile) {

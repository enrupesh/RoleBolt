...
export function ReviewCard({ review, featured = false }: { review: PublicReview; featured?: boolean }) {
  const isPartner = /plyndrox\.app|sendora\.me/i.test(review.displayName);
  return (
    <article className={`rounded-2xl border bg-white p-6 shadow-[0_10px_30px_rgba(32,79,112,.06)] ${featured || isPartner ? "border-[#b9d9f5]" : "border-[#dce7ef]"}`}>
      <div className="flex items-center justify-between gap-3">
        ...
        {(featured || isPartner) && (
          <span className="rounded-full bg-[#eaf3ff] px-2.5 py-1 text-[10px] font-bold uppercase tracking-[.12em] text-[#0a66c2]">
            {isPartner ? "Partner review" : "Featured by Rolebolt"}
          </span>
        )}
      </div>

  ...
  if (isLimit || isFeature || isRestricted) {
    ...
    return (
      <div className={`rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900 ${className}`}>
        ...
        {payload?.message && !isLimit ? (
          <p className="mt-1 text-xs text-amber-800">{payload.message}</p>
        ) : null}
        {payload?.upgradeRequired && (
          <a href="/billing?category=creator_form" className="mt-2 inline-block text-xs font-bold text-indigo-700 hover:text-indigo-800">
            View Form Job plans →
          </a>
        )}
      </div>

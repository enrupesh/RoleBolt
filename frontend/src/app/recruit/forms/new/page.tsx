...
function ShareModal({ slug, title, onClose }: { slug: string; title: string; onClose: () => void }) {
  ...
  const socials = [
    ...
    { name: "Twitter / X", href: `https://twitter.com/intent/tweet?text=${shareText}`, color: "bg-slate-800 hover:bg-slate-900", icon: "𝕏" },
            ...
            <div className="grid grid-cols-5 gap-2">
              {socials.map(s => (
                <a key={s.name} href={s.href} target="_blank" rel="noopener noreferrer"
                  className={`flex flex-col items-center gap-1 rounded-xl p-2.5 text-white transition ${s.color}`}
                  title={s.name}>

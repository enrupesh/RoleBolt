  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      await fetch(apiUrl("/recruit/auth/profile"), {
        ...
        body: JSON.stringify({ role: "seeker", name: data.user?.name, email: data.user?.email }),
      });
      ...
      <div className="w-full max-w-md">
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-8 shadow-[0_4px_24px_rgba(0,0,0,0.07)]">
          ...
          <p className="mt-6 text-center text-sm text-slate-500">
            ...
            <Link href="/seeker/signup" className="font-semibold text-indigo-600 hover:text-indigo-700">Create one free</Link>
          </p>
          <p className="mt-2 text-center text-xs text-slate-400">
            Looking to hire?{" "}
            <Link href="/recruit/login" className="text-slate-500 hover:text-slate-700 underline">Recruiter login →</Link>
          </p>
        </div>
      </div>
    </div>

...
Razorpay subscription Checkout shows a dynamic UPI QR as the documented desktop fallback. Mobile UPI Intent requires Razorpay-side enablement and a mobile-web-capable payment configuration; it cannot be created by adding a browser camera scanner.
...
**How to apply:** Prioritize the UPI method in Checkout options, optionally pass a server-exposed Razorpay Checkout Configuration ID, and explain desktop QR versus mobile UPI-app behavior in the pricing UI.